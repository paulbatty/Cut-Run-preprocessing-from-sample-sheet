...
# Debugging: Print the current replicate pair
echo "Processing Replicate Pair: $replicate1_cleaned, $replicate2_cleaned"
...
do
    ...
    #dups_removed_bam="${output_bam_merged_directory}/${merged_basename}_namesorted_fixmate_positionsorted_dupsremoved.bam" # This line is no longer needed

    # Submit a Slurm job for processing
    sbatch --partition=mediumq --qos=mediumq --nodes=1 --ntasks=1 --cpus-per-task=4 --time=2-00:00:00 --mem=8G --error="${merged_error_directory}/all_samples_${timestamp}.err" --output="${merged_log_directory}/all_samples_${timestamp}.log" --job-name=cnr5msam \
    --wrap="
    ...
    intersectBed -abam $positionsorted_bam -b /hg38/Homo_sapiens/UCSC/hg38/Sequence/Blacklist/CUTRun_Blacklist_hg38.bed -v > $output_bam_rm_bklist &&
