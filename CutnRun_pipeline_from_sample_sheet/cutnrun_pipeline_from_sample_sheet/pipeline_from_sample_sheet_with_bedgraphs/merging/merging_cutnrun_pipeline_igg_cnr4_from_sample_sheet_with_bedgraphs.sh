...
do
    ...
    sbatch --partition=mediumq --qos=mediumq --nodes=1 --ntasks=1 --cpus-per-task=4 --time=2-00:00:00 --mem=8G --error="${merged_error_directory}/all_samples_${timestamp}.err" --output="${merged_log_directory}/all_samples_${timestamp}.log" --job-name=cnr4migg \
    --wrap="
    ...
    intersectBed -abam $dups_removed_bam -b /hg38/Homo_sapiens/UCSC/hg38/Sequence/Blacklist/CUTRun_Blacklist_hg38.bed -v > $output_bam_rm_bklist &&
    ...
    # Step 9: Generate bedGraph file
    echo \"Processing merged barcode pair: $replicate1_cleaned-$replicate2_cleaned, Step 9: Generating bedGraph file...\"
    ...
    echo \"Processing merged barcode pair: $replicate1_cleaned-$replicate2_cleaned, Step 9: Generating bedGraph file finished.\"
