...
#SBATCH --mail-user=pbatty@cemm.oeaw.ac.at
...
output_bedgraph_merged_directory="$base_directory/out/merged_bedgraph"
...
mkdir -p "$output_bedgraph_merged_directory"
...
do
    ...
    output_bedgraph="${output_bedgraph_merged_directory}/${merged_basename}.bdg"
    ...
    sbatch --partition=mediumq --qos=mediumq --nodes=1 --ntasks=1 --cpus-per-task=4 --time=2-00:00:00 --mem=8G --error="${merged_error_directory}/all_samples_${timestamp}.err" --output="${merged_log_directory}/all_samples_${timestamp}.log" --job-name=cnr4msamp \
    --wrap="
    ...
    # Step 5: Remove reads overlapping with blacklist regions
    echo \"Processing merged barcode pair: $replicate1_cleaned-$replicate2_cleaned, Step 5: Removing reads overlapping with blacklist regions...\"
    intersectBed -abam $positionsorted_bam -b /hg38/Sequence/Blacklist/CUTRun_Blacklist_hg38.bed -v > $output_bam_rm_bklist &&
    echo \"Processing merged barcode pair: $replicate1_cleaned-$replicate2_cleaned, Step 5: Removing reads overlapping with blacklist regions finished.\"

    # Step 6: Index the final BAM
    echo \"Processing merged barcode pair: $replicate1_cleaned-$replicate2_cleaned, Step 6: Indexing the final BAM...\"
    samtools index $output_bam_rm_bklist &&
    echo \"Processing merged barcode pair: $replicate1_cleaned-$replicate2_cleaned, Step 6: Indexing the final BAM finished.\"

    # Step 7: Generate coverage tracks
    echo \"Processing merged barcode pair: $replicate1_cleaned-$replicate2_cleaned, Step 7: Generating coverage tracks...\"
    ...
    echo \"Processing merged barcode pair: $replicate1_cleaned-$replicate2_cleaned, Step 7: Generating coverage tracks finished.\"

    # Step 8: Generate bedGraph file
    echo \"Processing merged barcode pair: $replicate1_cleaned-$replicate2_cleaned, Step 8: Generating bedGraph file...\"
    ...
    echo \"Processing merged barcode pair: $replicate1_cleaned-$replicate2_cleaned, Step 8: Generating bedGraph file finished.\"
