...
do
    ...
    # Removing references to the output of the IgG duplicates step

    # Submit a Slurm job for processing
    sbatch --partition=mediumq --qos=mediumq --nodes=1 --ntasks=1 --cpus-per-task=4 --time=2-00:00:00 --mem=8G --error="${error_directory}/all_samples_${timestamp}.err" --output="${log_directory}/all_samples_${timestamp}.log" --job-name=cnr4sam \
    --wrap="
    ...
    # Step 7: Remove reads overlapping with blacklist regions
    echo \"Processing barcode: $barcode_cleaned, Step 7: Removing reads overlapping with blacklist regions...\"
    ...
    echo \"Processing barcode: $barcode_cleaned, Step 7: Removing reads overlapping with blacklist regions finished.\"

    # Step 8: Index the final BAM
    echo \"Processing barcode: $barcode_cleaned, Step 8: Indexing the final BAM...\"
    samtools index ${output_bam_rm_bklist} &&
    echo \"Processing barcode: $barcode_cleaned, Step 8: Indexing the final BAM finished.\"

    # Step 9: Generate coverage tracks
    echo \"Processing barcode: $barcode_cleaned, Step 9: Generating coverage tracks...\"
    ...
    echo \"Processing barcode: $barcode_cleaned, Step 9: Generating coverage tracks finished.\"

    # Step 10: Generate bedGraph file
    echo \"Processing barcode: $barcode_cleaned, Step 10: Generating bedGraph file...\"
    ...
    echo \"Processing barcode: $barcode_cleaned, Step 10: Generating bedGraph file finished.\"
