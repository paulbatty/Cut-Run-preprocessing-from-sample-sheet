...
do
    ...
    orig_bam_file="${output_bam_directory}/${barcode_cleaned}.bam"
    ...
    sbatch --partition=mediumq --qos=mediumq --nodes=1 --ntasks=1 --cpus-per-task=4 --time=2-00:00:00 --mem=8G --error="${error_directory}/all_samples_${timestamp}.err" --output="${log_directory}/all_samples_${timestamp}.log" --job-name=cnr4samp \
    --wrap="
    ...
    samtools view -bS $sam_file -o $orig_bam_file &&
    ...
    samtools sort -n $orig_bam_file -o $namesorted_bam &&
    ...
    intersectBed -abam $positionsorted_bam -b /Blacklist/CUTRun_Blacklist_hg38.bed -v > ${output_bam_rm_bklist} &&
